...
document.addEventListener("DOMContentLoaded", () => {
  ...
  .then((data) => {
    ...
    // ✅ MUOVILO QUI: listener che aggiorna selectedWorkout
    select.addEventListener("change", () => {
      selectedWorkout = workouts[select.value];
      document.getElementById("start-button").disabled = false;
    });

    // ✅ Se vuoi pre-selezionare il primo workout e abilitare il bottone:
...
function playExercise(index) {
  ...
  document.getElementById("video").src = exercise.videoUrl + "?autoplay=1";
